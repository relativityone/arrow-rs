...
pub trait FlightSqlService: Sync + Send + Sized + 'static {
    ...
    async fn do_get_fallback(
        ...
        Err(Status::unimplemented(format!(
            "do_get: The defined request is invalid: todogreg"
        )))
    }
    ...
    async fn do_put_fallback(
        ...
        Err(Status::unimplemented(format!(
            "do_put: The defined request is invalid: todogreg",
        )))
    }
    ...
    async fn do_get(
        ...
        match msg_result {
            ...
            Ok(msg) => match Command::try_from(msg).map_err(arrow_error_to_status)? {
                ...
                cmd => self.do_get_fallback(request).await,
            }
        }
    }
    ...
    async fn do_put(
        ...
        if let Some(Ok(cmd)) = cmd_opt {
            ...
            match message_result {
                Err(e) => self.do_put_fallback(request).await,
                Ok(message) => match Command::try_from(message).map_err(arrow_error_to_status)? {
                    ...
                    cmd => self.do_put_fallback(request).await,
                }
            }
        } else {
